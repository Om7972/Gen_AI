...
def process_youtube_video(video_link):
    ...
    try:
        ...
        try:
            ...
            if not os.path.exists(file_path):
                return {'error': 'Transcript file not found. Both transcript methods failed.'}
            ...
            if not paragraph or paragraph.strip() == "":
                return {'error': 'No text could be extracted from transcript'}
            ...
            if not summaries:
                return {'error': 'Failed to generate summary from any chunks. The transcript might be too short or the model failed to process it.'}
            ...
            return {'error': f'Transcript file not found: {str(e)}'}
        except ValueError as e:
            import traceback
            error_trace = traceback.format_exc()
            print(f"Value error: {e}")
            print(f"Traceback: {error_trace}")
            return {'error': f'Invalid transcript format: {str(e)}'}
        except Exception as e:
            import traceback
            error_trace = traceback.format_exc()
